...
screenshot_alt: "@sebastienrousseau/mocha-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/mocha-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/mocha-config — Shareable Mocha configuration rules for reliable, fast JavaScript and TypeScript test runners."
description: "Shareable Mocha configuration rules for reliable, fast JavaScript and TypeScript test runners."
keywords: "mocha-config, mocha, test runner, unit testing, assertion, javascript, typescript, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.6"
hero_tagline: "Reliable, standardized Mocha test runner configuration"
headline: "@sebastienrousseau/mocha-config"
lead: "Shareable Mocha configuration rules for reliable, fast JavaScript and TypeScript test runners."
cta_primary: "Install @sebastienrousseau/mocha-config"
...
snippet_caption: "Install @sebastienrousseau/mocha-config"
snippet_code: "npm install --save-dev @sebastienrousseau/mocha-config"
...
## What is @sebastienrousseau/mocha-config?

**@sebastienrousseau/mocha-config** is an enterprise-grade, shareable configuration for [Mocha](https://github.com/sebastienrousseau/mocha-config)—the feature-rich JavaScript test framework running on Node.js and in the browser. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14513) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/mocha-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/mocha-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/mocha-config` into any new or existing repository requires only three simple steps.
...
Create or update your `.mocharc.json` file at the root of your project to extend the shared configuration:

```json
{
  "extends": "@sebastienrousseau/mocha-config"
}
```

Learn how to customize specific rules and override settings in the [Usage Guide](usage/).

### Step 3: Run Mocha in Your Workflow

Execute Mocha directly or configure standard quality scripts in your `package.json`:

```bash
npx mocha
```

Run automated test suites with standardized reporters and timeout safeguards.
...
### Does this support modern ES Modules and TypeScript?
Yes. The configuration is tuned for native ESM test suites as well as TypeScript loaders (ts-node/esm or tsx), with deterministic test isolation.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `.mocharc.json` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/mocha-config safe for production CI/CD?
